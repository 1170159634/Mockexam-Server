...
public class AuthorizationController {
    ...
    public ResponseEntity<Object> login(@Validated @RequestBody AuthUserDto authUser, HttpServletRequest request) throws Exception {
        ...
        //记录到用户行为表
        MeUserBehavior userBehavior = meUserBehaviorService.getOne(new LambdaQueryWrapper<MeUserBehavior>().eq(MeUserBehavior::getUserId, jwtUserDto.getUser().getId()));
        if (userBehavior == null) {
            MeUserBehavior behavior = MeUserBehavior.builder().userId(jwtUserDto.getUser().getId())
                    ...
                    .isRead(0)
                    .build();
            meUserBehaviorService.save(behavior);
