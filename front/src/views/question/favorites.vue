...
import { userBehaviorSelectByPage, questionSelectById, questionChooseAnswer } from '@/api/system/question'
...
export default {
  ...
  cruds() {
    ...
    return CRUD({ title: '编辑', url: 'api/mockexam/user/behavior/selectByPage', params: { carType: 1 }, crudMethod: { ...crudQuestion }})
  },
  ...
  watch: {
    ...
    query: {
      handler(newVal, oldVal) {
        ...
        var params = { subjectType: this.query.subjectType, carType: 1 }
        userBehaviorSelectByPage(params).then((res) => {
  ...
  created() {
    var params = { subjectType: this.query.subjectType, carType: 1 }
    userBehaviorSelectByPage().then((res) => {
      this.questionIndexList = res.content
      this.questionCount = res.totalElements
      this.selectQuetsionById(this.questionIndexList[this.index])
    })
